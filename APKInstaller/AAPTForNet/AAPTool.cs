    ...
    public class AAPTool : System.Diagnostics.Process
    {
        ...
        public static ApkInfo Decompile(string path)
        {
            ...
            if(!infos.Any())
            {
                foreach (ApkInfos package in packages)
                {
                    List<ApkInfos> depandents = package.Apks.GroupBy(x => x.VersionCode).Select(x => new ApkInfos { VersionCode = x.Key, Apks = x.ToList() }).ToList();
                    foreach (ApkInfos depandent in depandents)
                    {
                        infos.Add(new ApkInfo 
                        {
                            AppName = "Depandents",
                            PackageName = depandent.PackageName,
                            VersionCode = depandent.VersionCode,
                            SplitApks = depandent.Apks
                        });
                    }
                }
            }

            ApkInfo info = infos.Count <= 1 ? infos.First() : new ApkInfo { AppName= "Multiple Packages", SplitApks = infos };
